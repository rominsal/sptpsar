...
build_G2d <- function (la, lg, nvarnopar = 0, dnoparlist = NULL,
  ...
  if (!is.null(g1u) & !is.null(g2u) & !is.null(g1b) & !is.null(g2b)) {
    ...
    if(!psanova){ # PSANOVA = FALSE
      ...
      g1v <- lg$g1v; g2v <- lg$g2v
      Ginv_eff <- NULL
      #Ginv <- c((1/la[2])*g1u, (1/la[3])*g2u,
      #          (1/la[4])*g1v, (1/la[5])*g2v, (1/la[4])*g1b + (1/la[5])*g2b)
      if(f1_main){
        Ginv_f1 <- (1/la[2])*g1u
  ...
  G_eff <- 1/Ginv_eff
  res <- list(G = G, Ginv = Ginv, G_eff = G_eff, Ginv_eff = Ginv_eff)
}
##########################################################################
...
build_G3d <- function (la, lg, nvarnopar = 0, dnoparlist = NULL,
                          ...
                          f12t_int=TRUE) {
    g1u <- lg$g1u; g2u <- lg$g2u; g3u <- lg$g3u
    g11b <- lg$g11b; g21b <- lg$g21b
    g12b <- lg$g12b; g31b <- lg$g31b
    g22b <- lg$g22b; g32b <- lg$g32b
    g1t <- lg$g1t; g2t <- lg$g2t; g3t <- lg$g3t
    ...
    if(!psanova){
        ...
        # NUEVA DEFINICIÓN DE Ginv Y Ginv_eff EN FUNCIÓN DE LAS INTERACCIONES INCLUIDAS
        g12u <- lg$g12u; g21u <- lg$g21u
        g13u <- lg$g13u; g31u <- lg$g31u; g13b <- lg$g13b
        g23u <- lg$g23u; g32u <- lg$g32u; g23b <- lg$g23b
        g123u <- lg$g123u; g213u <- lg$g213u; g321u <- lg$g321u
        g123b <- lg$g123b; g213b <- lg$g213b; g132b <- lg$g132b
        g312b <- lg$g312b; g231b <- lg$g231b; g321b <- lg$g321b
        Ginv_eff <- NULL
        if(f1_main){
            Ginv_f1 <- (1/la[2])*g1u
            Ginv_eff <- c(Ginv_eff,Ginv_f1)
        ...
        if (f1t_int) {
            ...
            Ginv_eff <- c(Ginv_eff,Ginv_f1t) } else {
                Ginv_f1t <- rep(0,length(g13u) + length(g31u) + length(g13b))
            }
        if (f2t_int) {
            Ginv_f2t <- c((1/la[9])*g23u, (1/la[10])*g32u,
                          (1/la[9])*g23b + (1/la[10])*g32b)
            Ginv_eff <- c(Ginv_eff,Ginv_f2t) } else {
                Ginv_f2t <- rep(0,length(g23u) + length(g32u) + length(g23b))
            }
        if (f12t_int) {
